...
class SaleOrder(models.Model):
    ...
    def _check_product(self, product, qty=1.0, lot=False, use_date=False):
        ...
        if order_line:
            ...
            if lot:
                lot_id = self.env['stock.production.lot'].search([('product_id', '=', product.id), ('name', '=', lot)])
            else:
                lot_id = False
            vals = {
    ...
    def on_barcode_scanned(self, barcode):
        ...
        if not self.company_id.sale_barcode_nomenclature_id:
            ...
            if parsed_result['type'] == 'lot':
                ...
                lot_id = self.env['stock.production.lot'].search(
                    [('name', '=', lot)])  # Logic by product but search by lot in existing lots
        ...
        return {'warning': {
            ...
            'message': _('The barcode "%(barcode)s" doesn\'t correspond to a proper product or lot.') % {
                'barcode': barcode}
        }}
